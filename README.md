...
Welcome to the **DevOps & Full-Stack Web Development** repository. This project contains hands-on labs, backend API services, and modern React web applications developed across weekly coursework.
...
| 📈 [`Week-8-Monday`](file:///Users/jashu/Documents/Devops/Week-8-Monday) | **Student Attendance Management System** | React, Context API, Modern UI Components | High-performance attendance management platform with analytics, KPI metric cards, filters, and state management. |

---

## 🛠 Tech Stack & Tools

- **Frontend**: React (v18 & v19), Vite, Vanilla CSS3, Lucide React Icons
- **Backend & Database**: Node.js, Express.js (v4 & v5), MongoDB, Mongoose
- **Security & Authentication**: JSON Web Tokens (JWT), BcryptJS password hashing, Middleware Guards
- **Development & Tooling**: npm, Oxlint, Postman Collections
...
- **MongoDB**: Local MongoDB server or MongoDB Atlas URI (for backend modules)
...
2. **Run a specific project module:**
   ...
   *For Frontend React Apps (`Week-5-Thursday`, `Week-6-Thursday`, `Week-7-Thursday`, `Week-8-Monday`):*
   ```bash
   cd Week-5-Thursday
   npm install
   npm run dev
   ...
   *For Backend Express Apps (`Week-5-Monday`, `Week-6-Monday`):*
   ```bash
   cd Week-6-Monday
   npm install
